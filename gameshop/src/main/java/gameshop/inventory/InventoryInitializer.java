...
class InventoryInitializer implements DataInitializer {
	...
	InventoryInitializer(Inventory<InventoryItem> inventory, GameCatalog gameCatalog) {
	...
	public void initialize() {
		...
		gameCatalog.findAll().forEach(game -> {

			// Try to find an InventoryItem for the project and create a default one with 10 items if none available
			inventory.findByProduct(game) //
					.orElseGet(() -> inventory.save(new InventoryItem(game, Quantity.of(10))));
		});
	}
}
