...
class OrderController {
	...
	/**
	 * Adds a {@link Game} to the {@link Cart}. Note how the type of the parameter taking the request parameter
	 * {@code pid} is {@link Game}. For all domain types extening {@link AbstractEntity} (directly or indirectly) a tiny
	 ...
	 */
	@PostMapping("/cart")
	String addDisc(@RequestParam("pid") Game game, @RequestParam("number") int number, @ModelAttribute Cart cart) {
		...
		cart.addOrUpdateItem(game, Quantity.of(amount));

		// (｡◕‿◕｡)
		// Je nachdem ob disc eine SINGLE oder eine Bluray ist, leiten wir auf die richtige Seite weiter
